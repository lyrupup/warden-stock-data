...
func (r *JobRepository) EnsureDefaults(ctx context.Context) error {
	// 三类作业：证券列表 / 增量日K / 全量日K（Python baostock 采集）；指标实时计算、不再落库。
	defaults := []model.UpdateJob{
		...
		{
			// 交易日历每年增量发布，每月 1 日 8:00 刷新一次即可（baostock 官方日历）。
			Name: "calendar-sync", JobType: "calendar", Market: "CN",
			CronExpr: "0 0 8 1 * *", BatchSize: 1, Concurrency: 1, Enabled: true,
		},
		{
		...
		{
			// 周级 baostock 对齐：用 baostock 覆盖区间日 K（source 翻 baostock）+复权因子+证券列表。
			// 手动触发须选区间；cron 无区间时缺省对齐最近 7 天。周六 5:00。
			Name: "stock-weekly-refresh", JobType: "factors", Market: "CN",
			CronExpr: "0 0 5 * * 6", BatchSize: 20, Concurrency: 5, Enabled: true,
		},
	}
...
// EnsureDataSource 保证存在指定 source 的数据源记录（缺失则创建），用于登记
// 非默认主行情源之外的数据源（如 baostock 日 K 采集源），以便在运维页统一展示与探测。
func (r *JobRepository) EnsureDataSource(ctx context.Context, source, name string, priority int) error {
	if source == "" {
		return nil
	}
	var existing model.DataSource
	...
	return r.db.WithContext(ctx).Create(&model.DataSource{
		Source: source, Market: "CN", Name: name, Enabled: true, Priority: priority, Health: "unknown",
	}).Error
}
