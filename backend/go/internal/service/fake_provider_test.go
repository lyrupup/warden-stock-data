...
// buildKlineBars 由收盘价序列构造升序日 K（自 2024-01-01 起逐日），供分页窗口等纯函数测试使用。
func buildKlineBars(closes []float64) []model.StockDailyKline {
	base := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	bars := make([]model.StockDailyKline, 0, len(closes))
	for i, c := range closes {
		price := decimal.NewFromFloat(c)
		bars = append(bars, model.StockDailyKline{
			Market: "CN", Source: "fake", StockCode: "600000",
			TradeDate: base.AddDate(0, 0, i),
			Open:      price, High: price, Low: price, Close: price, Adjust: "qfq",
		})
	}
	return bars
}
