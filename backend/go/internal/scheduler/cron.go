...
func (s *CronScheduler) runScheduled(ctx context.Context, job *model.UpdateJob) {
	...
	// 交易日历感知仅适用于「拉 K 线」的作业（full/incremental）；
	// 元数据类作业（securities/calendar）需在非交易日也能执行（如月初节假日刷新日历）。
	if job.JobType == JobKlineFull || job.JobType == JobKlineIncremental || job.JobType == "" {
		today := time.Now()
