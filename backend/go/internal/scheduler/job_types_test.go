...
func TestCollectModeOf(t *testing.T) {
	require.Equal(t, "full", collectModeOf(JobKlineFull))
	require.Equal(t, "incremental", collectModeOf(JobKlineIncremental))
	require.Equal(t, "incremental", collectModeOf(""))
	require.Equal(t, "incremental", collectModeOf("unknown_type"))
	require.Equal(t, collectModeNoop, collectModeOf("indicator_full"))
	require.Equal(t, collectModeNoop, collectModeOf("snapshot"))
}
